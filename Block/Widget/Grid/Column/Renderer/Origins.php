<?php
namespace Hexasoft\IP2LocationCountryBlocker\Block\Widget\Grid\Column\Renderer;
...
class Origins extends AbstractRenderer
{
	/**
	 * @var Registry
	 */
	...
	public function _getValue(\Magento\Framework\DataObject $row)
	{
		...
		$values = explode(';', parent::_getValue($row));
		...
		foreach ($values as $value) {
			...
			if (strpos($value, '-')) {
				list($countryCode, $regionName) = explode('-', $value);
				...
				if (isset($countries[$countryCode])) {
					if ($regionName == '*') {
						...
						$result[] = $countries[$countryCode] . ' - ' . $regionName;
					}
				}
			}
		}
